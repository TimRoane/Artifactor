...
For targeted-NGS coverage, negative-binomial target models use sequencing-exposure and target-length offsets plus shrunk targetwise dispersion. Observed counts remain distinct from log offset-normalized, median-ratio, GC-adjusted, and technical-residual exploratory representations. Eligibility requires identifiable design, material chance-adjusted technical-predictability reduction, less than five percent protected-biological loss, and preserved replicate agreement.

Allele evidence always retains numerator and denominator. Callable observations use binomial GLMs and a quasi-binomial overdispersion diagnostic; sparse observations remain explicitly insufficient. Context, FFPE, strand/orientation, and optional upstream read-support summaries are evaluated with depth and callability context. These diagnostics never rewrite a VAF or call.
